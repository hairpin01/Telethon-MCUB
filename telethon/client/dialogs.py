    ...
    async def _init(self, entities, **kwargs):
        if not entities:
            ...
            for entity in entities:
                peers.append(types.InputDialogPeer(await self.client.get_input_entity(entity)))
        ...
        self.buffer.extend(
            custom.Draft(self.client, entities[utils.get_peer_id(d.peer)], d.draft) for d in items
        )
...
class DialogMethods:
    ...
    def iter_drafts(self: "TelegramClient", entity: "hints.EntitiesLike" = None) -> _DraftsIter:
        """
        ...
        *,
        ...
        return await self(
            functions.folders.EditPeerFoldersRequest(
                [types.InputFolderPeer(x, folder_id=y) for x, y in zip(entities, folder)]
            )
        )
