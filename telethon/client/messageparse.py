...
class MessageParseMethods:
    ...
    async def _replace_with_mention(self: "TelegramClient", entities, i, user):
        ...
        try:
            entities[i] = types.InputMessageEntityMentionName(
                entities[i].offset, entities[i].length, await self.get_input_entity(user)
            )
            return True
    ...
    def _get_response_message(self: "TelegramClient", request, result, input_chat):
        ...
        if isinstance(result, types.UpdateShort):
            ...
            entities = {
                utils.get_peer_id(x): x for x in itertools.chain(result.users, result.chats)
            }
        else:
            return None
        ...
        for update in updates:
            if isinstance(update, types.UpdateMessageID):
            ...
            elif isinstance(update, (types.UpdateNewChannelMessage, types.UpdateNewMessage)):
            ...
            elif isinstance(update, types.UpdateEditChannelMessage) and utils.get_peer_id(
                request.peer
                ...
                if request.media.poll.id == update.poll_id:
                    m = types.Message(
                        ...
                        media=types.MessageMediaPoll(poll=update.poll, results=update.results),
                    )
        ...
        random_id = (
            request if isinstance(request, (int, list)) else getattr(request, "random_id", None)
        )
