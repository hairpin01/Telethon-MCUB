...
class Conversation(ChatGetter):
    ...
    def get_response(self, message=None, *, timeout=None):
        ...
        return self._get_message(
            message, self._response_indices, self._pending_responses, timeout, lambda x, y: True
        )
    ...
    def get_edit(self, message=None, *, timeout=None):
        ...
        earliest_edit = min(
            (
                ...
                if x.edit_date and x.id > target_id and x.edit_date.timestamp() > target_date
            ),
    ...
    def _get_result(self, future, start_time, timeout, pending, target_id):
        ...
        return asyncio.wait_for(future, timeout=None if due == float("inf") else due - time.time())
